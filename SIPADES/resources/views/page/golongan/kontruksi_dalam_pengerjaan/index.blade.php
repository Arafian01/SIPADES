<x-app-layout>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center gap-2">
                            ...
                            <div class="relative w-full sm:w-64">
                                ...
                                <input type="text" id="searchInput" placeholder="Cari kontruksi..."
                            ...
                            <!-- Add Button -->
                            <a href="{{ route('kontruksi_dalam_pengerjaan.create', '0') }}"
                ...
                <div class="p-4 sm:p-6 text-gray-900 dark:text-gray-100">
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <table id="kontruksiTable" class="w-full text-sm text-left rtl:text-right">
                            ...
                            <tbody id="kontruksiTableBody">
                                @php $no = 1; @endphp
                                @foreach ($kontruksi_dalam_pengerjaan as $a)
                                    <tr class="kontruksi-row bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        ...
                                        <td class="px-4 py-3">{{ $a->aset->nomor_register }}</td>
                                        <td class="px-4 py-3 font-mono">{{ $a->aset->rekening->kode }}</td>
                                        <td class="px-4 py-3">{{ $a->aset->rekening->nama_rekening }}</td>
                                        <td class="px-4 py-3">Rp {{ number_format($a->aset->nilai_perolehan, 2, ',', '.') }}</td>
                                        <td class="px-4 py-3 flex flex-wrap justify-center gap-2">
                                            <a href="{{ route('kontruksi_dalam_pengerjaan.edit', [$a->id, '0']) }}"
                                            ...
                                            <button onclick="kontruksiDelete('{{ $a->id }}','{{ addslashes($a->aset->rekening->nama_rekening) }}')"
                    ...
                    <div class="mt-4 flex justify-center">
                        {{ $kontruksi_dalam_pengerjaan->links() }}
                    </div>
                    <!-- Empty State -->
    ...
    <script>
        ...
        function searchTable() {
            ...
            const rows = document.getElementsByClassName('kontruksi-row');
        ...
        async function kontruksiDelete(id, nama) {
            const result = await Swal.fire({
                title: `Hapus Kontruksi ${nama}?`,
            ...
            if (result.isConfirmed) {
                try {
                    await axios.post(`/kontruksi_dalam_pengerjaan/${id}`, {
                        '_method': 'DELETE',
                        '_token': '{{ csrf_token() }}'
                    });
                    Swal.fire({
                        title: 'Terhapus!',
                        text: 'Data kontruksi telah dihapus.',
                        icon: 'success',
                        timer: 2000,
                        showConfirmButton: false
        ...
        document.addEventListener('DOMContentLoaded', function () {
            const tbody = document.getElementById('kontruksiTableBody');
            const emptyState = document.getElementById('emptyState');
