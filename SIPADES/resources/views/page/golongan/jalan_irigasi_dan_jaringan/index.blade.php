<x-app-layout>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-4 sm:p-6 text-gray-900 dark:text-gray-100">
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <table id="jalanTable" class="w-full text-sm text-left rtl:text-right">
                            ...
                            <tbody id="jalanTableBody">
                                ...
                                @foreach ($jalan_irigasi_dan_jaringan as $j)
                                    <tr class="jalan-row bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ $no++ }}</td>
                                        <td class="px-4 py-3 font-mono text-blue-600 dark:text-blue-400">{{ $j->aset->id_barang }}</td>
                                        <td class="px-4 py-3">{{ $j->aset->nomor_register }}</td>
                                        <td class="px-4 py-3 font-mono">{{ $j->aset->rekening->kode }}</td>
                                        <td class="px-4 py-3">{{ $j->aset->rekening->nama_rekening }}</td>
                                        <td class="px-4 py-3">{{ number_format($j->luas, 2, ',', '.') }} m²</td>
                                        <td class="px-4 py-3">Rp {{ number_format($j->aset->nilai_perolehan, 2, ',', '.') }}</td>
                                        <td class="px-4 py-3 flex flex-wrap justify-center gap-2">
                                            ...
                                            <button onclick="jalanDelete('{{ $j->id }}','{{ addslashes($j->aset->rekening->nama_rekening) }}')"
                    ...
                    <div class="mt-4 flex justify-center">
                        {{ $jalan_irigasi_dan_jaringan->links() }}
                    </div>
                    <!-- Empty State -->
    ...
    <script>
        ...
        function searchTable() {
            ...
            const rows = document.getElementsByClassName('jalan-row');
        ...
        async function jalanDelete(id, nama) {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            const tbody = document.getElementById('jalanTableBody');
            const emptyState = document.getElementById('emptyState');
