<x-app-layout>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-4 sm:p-6 text-gray-900 dark:text-gray-100">
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <table id="gedungTable" class="w-full text-sm text-left rtl:text-right">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-300 sticky top-0">
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-3 min-w-[120px]">ID Aset</th>
                                    <th scope="col" class="px-4 py-3">No Reg</th>
                                    <th scope="col" class="px-4 py-3">Kode Rekening</th>
                                    <th scope="col" class="px-4 py-3 min-w-[180px]">Nama Rekening</th>
                                    <th scope="col" class="px-4 py-3">Luas (m²)</th>
                            ...
                            <tbody id="gedungTableBody">
                                ...
                                @foreach ($gedung_dan_bangunan as $t)
                                    <tr class="gedung-row bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        ...
                                        <td class="px-4 py-3 font-mono">{{ $t->aset->nomor_register }}</td>
                                        <td class="px-4 py-3 font-mono">{{ $t->aset->rekening->kode }}</td>
                                        <td class="px-4 py-3">{{ $t->aset->rekening->nama_rekening }}</td>
                                        <td class="px-4 py-3 font-mono">{{ number_format($t->luas, 2, ',', '.') }} m²</td>
                                        <td class="px-4 py-3 font-mono">Rp {{ number_format($t->aset->nilai_perolehan, 2, ',', '.') }}</td>
                                        <td class="px-4 py-3 flex flex-wrap justify-center gap-2">
                                            ...
                                            <button onclick="gedungDelete('{{ $t->id }}','{{ addslashes($t->aset->rekening->nama_rekening) }}')"
    ...
    <script>
        ...
        function searchTable() {
            ...
            const rows = document.getElementsByClassName('gedung-row');
        ...
        async function gedungDelete(id, nama) {
            ...
            if (result.isConfirmed) {
                try {
                    await axios.post(`/gedung_dan_bangunan/${id}`, {
                        '_method': 'DELETE',
                        '_token': '{{ csrf_token() }}'
                    });
                    Swal.fire({
                        title: 'Terhapus!',
                        text: 'Data gedung telah dihapus.',
                        icon: 'success',
                        timer: 2000,
                        showConfirmButton: false
        ...
        document.addEventListener('DOMContentLoaded', function () {
            const tbody = document.getElementById('gedungTableBody');
            const emptyState = document.getElementById('emptyState');
