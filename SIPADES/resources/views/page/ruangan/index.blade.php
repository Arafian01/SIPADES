<x-app-layout>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-4 sm:p-6 text-gray-900 dark:text-gray-100">
                    <div class="relative overflow-x-auto">
                        <table id="ruanganTable" class="w-full text-sm text-left rtl:text-right">
                            ...
                            <tbody id="ruanganTableBody">
                                ...
                                @foreach ($ruangan as $r)
                                    <tr class="ruangan-row bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ $no++ }}</td>
                                        <td class="kode-ruangan px-4 py-3 font-mono text-blue-600 dark:text-blue-400">{{ $r->kode_ruangan }}</td>
                                        <td class="nama-ruangan px-4 py-3">{{ $r->nama_ruangan }}</td>
                                        <td class="px-4 py-3">
                                        ...
                                        <td class="px-4 py-3 flex flex-wrap justify-center gap-2">
                                            <button type="button" data-id="{{ $r->id }}"
                                                ...
                                                data-nama_ruangan="{{ $r->nama_ruangan }}" data-id_pengguna="{{ $r->id_pengguna }}"
                                            ...
                                            <button onclick="ruanganDelete('{{ $r->id }}','{{ $r->nama_ruangan }}')"
    ...
    <div class="fixed inset-0 flex items-center justify-center z-50 hidden" id="sourceModal">
        ...
        <div class="flex items-center justify-center p-4 w-full">
            <div class="w-full max-w-md sm:max-w-lg relative bg-white rounded-lg shadow-xl dark:bg-gray-800">
                ...
                <form method="POST" id="formSourceModal" action="{{ route('ruangan.store') }}">
                    @csrf
                    <div class="p-4 sm:p-6 space-y-4">
                        <div>
                            <label for="kode_ruangan" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
        ...
        <div class="flex items-center justify-center p-4 w-full">
            <div class="w-full max-w-md sm:max-w-lg relative bg-white rounded-lg shadow-xl dark:bg-gray-800">
                ...
                <form method="POST" id="formSourceModalEdit">
                    ...
                    <div class="p-4 sm:p-6 space-y-4">
                        <div>
                            <label for="kode_ruangan_edit" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                <svg class="w-4 h-4 mr-2 inline text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14"></path>
                                </svg>
                                Kode Ruangan
                            </label>
                            <input type="text" id="kode_ruangan_edit" name="kode_ruangan" placeholder="Masukkan kode ruangan"
                        ...
                        <div>
                            <label for="nama_ruangan_edit" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                <svg class="w-4 h-4 mr-2 inline text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                                Nama Ruangan
                            </label>
                            <input type="text" id="nama_ruangan_edit" name="nama_ruangan" placeholder="Masukkan nama ruangan"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-amber-500 focus:border-amber-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-amber-500 dark:focus:border-amber-500"
                                required />
                        </div>
                        <div>
    ...
    <script>
        ...
        function searchTable() {
            ...
            const rows = document.getElementsByClassName('ruangan-row');
            const emptyState = document.getElementById('emptyState');
            let hasResults = false;

            for (let row of rows) {
                const kode = row.getElementsByClassName('kode-ruangan')[0].textContent.toLowerCase();
                const nama = row.getElementsByClassName('nama-ruangan')[0].textContent.toLowerCase();
                const penanggungJawab = row.querySelector('td:nth-child(4) span').textContent.toLowerCase();

                if (kode.includes(input) || nama.includes(input) || penanggungJawab.includes(input)) {
                    row.style.display = '';
                    hasResults = true;
                } else {
        ...
        function editSourceModal(button) {
            ...
            formModal.action = "{{ route('ruangan.update', ':id') }}".replace(':id', id);
        ...
        async function ruanganDelete(id, ruangan) {
        ...
        $(document).ready(function() {
            $('.js-example-placeholder-single').select2({
                placeholder: "Pilih Penanggung Jawab (Opsional)",
                allowClear: true,
                width: '100%'
            });
        });
